...
def groupcount_differences_between_rows(
    ...
    for (i, row1), (_, row2) in zip(df1.iterrows(), df2.iterrows()):
        ...
        if group:
            diffs = diffs.append(
                dict(group=tuple(group), count=1, indices=[i]), ignore_index=True
            )
    diffs = diffs.groupby("group", as_index=False).agg(
        {"count": "sum", "indices": "sum"}
    )
    return diffs


def get_names_of_unequal_fields(row1: pd.Series, row2: pd.Series) -> List[str]:
    return [
        str(name)
        for (name, field1), (_, field2) in zip(row1.items(), row2.items())
        if field1 != field2
    ]
...
def check_and_parse_dataframes(
    ...
    if df1.shape != df2.shape:
        raise ValueError(
            f"df1 and df2 do not have same shape: {df1.shape}, {df2.shape}"
        )
    for df in (df1, df2):
        if len(df.columns) != len(set(df.columns)):
            raise ValueError(
                f"Dataframes are not allowed to have non-unique columns: {df1.columns}, {df2.columns}"
            )
    if set(df1.columns) != set(df2.columns):
        raise ValueError(
            f"df1 and df2 do not have the same set of columns: {df1.columns}, {df2.columns}"
        )
